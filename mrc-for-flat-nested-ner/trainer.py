...
class BertLabeling(pl.LightningModule):
    ...
    def __init__(
        ...
        self.result_dir = args.default_root_dir
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--mrc_dropout", type=float, default=0.2,
        ...
        parser.add_argument("--flat", action="store_false", help="is flat ner")
        ...
        parser.add_argument("--chinese", action="store_false",
    ...
    def training_step(self, batch, batch_idx):
        ...
        start_logits, end_logits, span_logits = self(tokens, attention_mask, token_type_ids) # 调用forward
    ...
    def validation_step(self, batch, batch_idx):
        ...
        output[f"start_preds"] = start_preds
        output[f"end_preds"] = end_preds
    ...
    def test_epoch_end(
        ...
        for x in outputs:
            start_preds.extend(x['start_preds'].detach().cpu().numpy().tolist())
    ...
    def get_dataloader(self, prefix="train", limit: int = None) -> DataLoader:
        ...
        load_mmap_dataset
        """
        json_path = os.path.join(self.data_dir, f"cys_mrc.{prefix}")
        vocab_path = os.path.join(self.bert_dir, "vocab.txt")
        dataset = MRCNERDataset(json_path=json_path,
                                tokenizer=BertTokenizer.from_pretrained(self.bert_dir, do_lower_case=False),
...
def main():
    ...
    # args.default_root_dir = "/opt/hyp/NER/Cysecurity_pretrain/mrc-for-flat-nested-ner/result"
    # args.precision = 32
