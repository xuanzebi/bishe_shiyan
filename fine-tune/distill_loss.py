...
def patience_loss(state_S, state_T, normalized_patience=True):
    # n_batch = teacher_patience.shape[0]
    if normalized_patience:
        state_S = F.normalize(state_S, p=2, dim=2)
        state_T = F.normalize(state_T, p=2, dim=2)
    return F.mse_loss(state_T.float(), state_S.float())
...
# b= a[:,0]
# print(b)
# c = F.normalize(a,p=2,dim=2)
# print(c)

# input = torch.randn(3, 5)
# target = torch.randn(3, 5)
# output = F.mse_loss(input, target,reduction='none').view(-1).mean()
# print(F.mse_loss(input, target))
# print(output)